...
+ (instancetype)crashLoggerWithName: (NSString *)name
    ...
    YPCrashLogger * crashLogger = [YPCrashLogger new];
...
@implementation YPCrashLoggerServer


#pragma mark - Singleton
+ (instancetype)sharedServer {
    static YPCrashLoggerServer * sharedServer;
...
- (void)fetchLoggers: (void(^)(NSArray<YPCrashLogger *> *))fetchHandle {
    [self _syncExecute: ^{
...
- (void)_syncExecute: (dispatch_block_t)block {
    ...
    if ([NSThread isMainThread]) {
        dispatch_async(YPDispatchQueueGetForQOS(NSQualityOfServiceUtility), block);
    } else {
        block();
    }
}

- (void)_insertCrashLogger: (YPCrashLogger *)crashLogger {
...
- (void)_fetchLastLogger: (void(^)(YPCrashLogger *))fetchHandle {
...
- (void)_fetchCrashLoggers: (void(^)(NSArray<YPCrashLogger *> *))fetchHandle {
    assert(fetchHandle != nil);
...
- (YPCrashLogger *)_dbGetLoggerFromStmt: (sqlite3_stmt *)stmt {
    int idx = 0;
    ...
    YPCrashLogger * logger = [YPCrashLogger new];
